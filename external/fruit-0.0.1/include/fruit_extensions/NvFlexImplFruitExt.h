#ifndef NV_FLEX_IMPL_FRUIT_EXT_H
#define NV_FLEX_IMPL_FRUIT_EXT_H

#include <flex/NvFlex.h>
#include <flex/NvFlexExt.h>

#include <fruit/fruit_ext.h>
...
public:
	...
	NvFlexBuffer* GetNvBuffer() const {
		return buffer;
	}
	...
	void map(int flags = eNvFlexMapWait) {
		...
		assert(!mappedPtr);
		mappedPtr = reinterpret_cast<T*>(NvFlexMap(buffer, flags));
	}
