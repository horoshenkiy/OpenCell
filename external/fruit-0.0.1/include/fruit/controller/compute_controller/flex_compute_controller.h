...
class FlexComputeController : public Compute::ComputeController {
public:
	...
	bool IsInitialize() override {
		return flexController->GetSolver();
	}
	...
	void Update() {
		...
		if (!AppParams::g_pause || AppParams::g_step)
		{
			...
			NvFlexUpdateSolver(flexController->GetSolver(), flexParams->dt, flexParams->numSubsteps, flexParams->profile);

			AppParams::g_frame++;
			AppParams::g_step = false;
		}
