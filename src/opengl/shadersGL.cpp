...
void StartFrame(Vec4 clearColor)
{
	...
	glVerify(glBindFramebuffer(GL_DRAW_FRAMEBUFFER_EXT, renderParam->msaaFbo));
...
void EndFrame()
{
	if (renderParam->msaaFbo)
	{
		...
		glVerify(glBindFramebuffer(GL_READ_FRAMEBUFFER_EXT, renderParam->msaaFbo));
		glVerify(glBindFramebuffer(GL_DRAW_FRAMEBUFFER_EXT, 0));
		glVerify(glBlitFramebuffer(0, 0, renderController.GetWidth(), renderController.GetHeight(), 0, 0, renderController.GetWidth(), renderController.GetHeight(), GL_COLOR_BUFFER_BIT, GL_LINEAR));
	}
...
void PresentFrame(bool fullsync)
{
	...
	glFinish();
	SDL_GL_SwapWindow(renderController.GetWindow());
}
...
{
	glVerify(glViewport(0, 0, renderController.GetWidth(), renderController.GetHeight()));
...
void ReflectEnd(ReflectMap* map, int width, int height)
{
	...
	glBindFramebuffer(GL_FRAMEBUFFER, renderParam->msaaFbo);

	glViewport(0, 0, renderController.GetWidth(), renderController.GetHeight());
}
...
void RenderEllipsoids(FluidRenderer* render, FluidRenderBuffers buffers, int n, int offset, float radius, float screenWidth, float screenAspect, float fov, Vec3 lightPos, Vec3 lightTarget, Matrix44 lightTransform, ShadowMap* shadowMap, Vec4 color, float blur, float ior, bool debug)
{
	...
	if (renderBuffers->mesh)
		DrawMesh(renderBuffers->mesh, Vec3(1.0f));

	renderController.DrawShapes();
